...
const SERVICE_NAME = 'frontend-react';
const SERVICE_VERSION = '0.1.0';
const STACK = 'react';

const OTLP_ENDPOINT =
  (import.meta.env?.VITE_OTLP_ENDPOINT as string) || 'http://localhost:4318';
const FARO_URL =
  (import.meta.env?.VITE_FARO_URL as string) || 'http://localhost:12347/collect';
const REPLAY_ENDPOINT =
  (import.meta.env?.VITE_REPLAY_ENDPOINT as string) || 'http://localhost:8080/replay/ingest';
...
let postInteractionsCounter: any;
let postDwellHistogram: any;
let scrollDepthGauge: any;
let storyDwellHistogram: any;
let storyViewsCounter: any;
let storySkipsCounter: any;
let tabSwitchesCounter: any;
const webVitalGauges: Record<string, any> = {};

export function getMeters() {
  return {
    postInteractionsCounter,
    postDwellHistogram,
    scrollDepthGauge,
    storyDwellHistogram,
    storyViewsCounter,
...
export function emitLog(body: Record<string, unknown>) {
  try {
...
export function initTelemetry() {
  try {
    ...
    storyDwellHistogram = meter.createHistogram('story_dwell_time_ms');
    storyViewsCounter = meter.createCounter('story_views_total');
    storySkipsCounter = meter.createCounter('story_skips_total');
    tabSwitchesCounter = meter.createCounter('tab_switches_total');
...
function initWebVitals() {
  const record = (name: string) => (m: { value: number }) => {
    try {
...
function sendWithRetry(body: string, attempt: number) {
